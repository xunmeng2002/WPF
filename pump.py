...
if __name__ == "__main__":
        ...
        for match in itr:
				...
				#if match.start() > 0 and tpl_content[match.start()-1] == "\n" \
                #        and len(tpl_content) > match.end() and tpl_content[match.end()] == "\n":
                #        common_str = tpl_content[last_pos:match.start()-1]
                #else:
                #        common_str = tpl_content[last_pos:match.start()]             

                #检查match是不是!!travel * !!加换行
                #if re.compile("!!travel .*?!!").match(tpl_content[match.start():match.end() + 1]):
                #        last_pos = match.end() + 1
                #else:
                #        last_pos = match.end()

                #if (tpl_content[match.end()] == "\n"):
                #        last_pos = match.end() + 1
                #else:
                #       last_pos = match.end()
                last_pos = match.end()
